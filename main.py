...
#mqtt_Client_ID = f'raspberry-pub-{time.time_ns()}' 
#mqtt_user      = "emqx"
#mqtt_password  = "public"
#topic          = "Wi-Sun-Hat/Shibayama/mqtt"
#msg            = b'{"msg":"hello"}'
...
def draw_w():
    ...
    if Disp_mode == 1 : # 表示回転処理
        if m5type == 0 :
            ...
            lcd.font(lcd.FONT_Default, rotate = 90) # 単位(W)の表示
            lcd.print('W', 26, 144, fc)
            if len(str(u.instant_power[0])) == 4:
                lcd.font(lcd.FONT_DejaVu56, rotate = 90) # 瞬時電力値の表示
                lcd.print(str(u.instant_power[0]), 55, 10, fc)
            else:
                lcd.font(lcd.FONT_DejaVu72, rotate = 90) # 瞬時電力値の表示
                lcd.print(str(u.instant_power[0]), 60, 10, fc)
        if m5type == 1 :
            lcd.rect(0, 0, 111, 240, lcd.BLACK, lcd.BLACK)
            lcd.font(lcd.FONT_DejaVu24, rotate = 90) # 単位(W)の表示
            lcd.print('W', 26, 200, fc)
            if len(str(u.instant_power[0])) == 4:
                lcd.font(lcd.FONT_DejaVu72, rotate = 90) # 瞬時電力値の表示
                lcd.print(str(u.instant_power[0]), 80, 30, fc)
            else:
                lcd.font(lcd.FONT_DejaVu72, rotate = 90) # 瞬時電力値の表示
                lcd.print(str(u.instant_power[0]), 80, 50, fc)
            lcd.font(lcd.FONT_Default, rotate = 90)
            lcd.print('10days',              110, 10, fc)
            lcd.print(str(u.price[10])+'Yen',100, 10, fc)
            lcd.print('20days',              110, 90, fc)
            lcd.print(str(u.price[20])+'Yen',100, 90, fc)
            lcd.print('30days',              110, 170, fc)
            lcd.print(str(u.price[30])+'Yen',100, 170, fc)
    else :
        if m5type == 0 :
            lcd.rect(15 , 0, 80, 160, lcd.BLACK, lcd.BLACK)
            lcd.font(lcd.FONT_Default, rotate = 270) # 単位(W)の表示
            lcd.print('W', 60, 20, fc)
            if len(str(u.instant_power[0])) == 4:
                lcd.font(lcd.FONT_DejaVu56, rotate = 270) # 瞬時電力値の表示
                lcd.print(str(u.instant_power[0]), 30, 100, fc)
            else:
                lcd.font(lcd.FONT_DejaVu72, rotate = 270) # 瞬時電力値の表示
                lcd.print(str(u.instant_power[0]), 20, 100, fc)
        if m5type == 1 :
            lcd.rect(24 , 0, 135, 240, lcd.BLACK, lcd.BLACK)
            lcd.font(lcd.FONT_DejaVu24, rotate = 270) # 単位(W)の表示
            lcd.print('W', 100, 30, fc)

            if len(str(u.instant_power[0])) == 4:
                lcd.font(lcd.FONT_DejaVu72, rotate = 270) # 瞬時電力値の表示
                lcd.print(str(u.instant_power[0]), 50, 160, fc)
            else:
                lcd.font(lcd.FONT_DejaVu72, rotate = 270) # 瞬時電力値の表示
                lcd.print(str(u.instant_power[0]), 50, 160, fc)
    ...
    if ((utime.time() - tp_c) >= (30 * 60)) or ((not tp_f) and ((utime.time() - tp_c) >= 10)) : # 30分毎に積算電力量要求コマンド送信（受信出来ない時のコマンド再送信は10秒開ける）
        ...
        if i%5000==1:
            ...
            if collect_day>30:
                collect_day=10
                i=0
            POWER_W = b'\x10\x81\x00\x01\x05\xFF\x01\x02\x88\x01\x61\x01\xE5\x01'+bytes([collect_day])           #E5
            ...
            POWER_R = b'\x10\x81\x00\x01\x05\xFF\x01\x02\x88\x01\x62\x01\xE2\x00'           #E2
            ...
            POWER_R = b'\x10\x81\x00\x01\x05\xFF\x01\x02\x88\x01\x62\x01\xE8\x00'           #E8
            command = bytes('SKSENDTO 1 {0} 0E1A 1 {1:04X} '.format(ipv6Addr, len(POWER_R)), 'utf-8')
            bp35a1.write(command)
            bp35a1.write(POWER_R)
